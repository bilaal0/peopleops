...
export async function loader({ params, request }) {
  ...
  if (!clientUser) {
    throw new Response("Client User Not Found", { status: 404 });
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <Link
                to={`/user-accounts/client/${client._id}/edit`}
              ...
              <Form method="post" action={`/user-accounts/client/${client._id}/delete`} onSubmit={(e) => {
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-6 shadow-xs space-y-6">
              ...
              <div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                    isCompany ? "bg-purple-100 text-purple-800" : "bg-blue-100 text-blue-800"
                  }`}>
                    {isCompany ? "Company Client" : "Individual Client"}
                  </span>
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                    client.status === 1 ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"
                  }`}>
            ...
            <div>
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                  <dt className="text-gray-500 text-xs">Position / Role</dt>
                  <dd className="font-medium text-gray-900 mt-0.5">{client.positionInCompany || "—"}</dd>
                </div>
                {isCompany && (
                  <>
                    <div>
                      <dt className="text-gray-500 text-xs">Company Name</dt>
                      <dd className="font-medium text-gray-900 mt-0.5">{client.landlordData?.companyName || "—"}</dd>
                    </div>
                    <div>
                      <dt className="text-gray-500 text-xs">Company Number</dt>
                      <dd className="font-medium text-gray-900 mt-0.5">{client.landlordData?.companyNumber || "—"}</dd>
                    </div>
                  </>
                )}
                <div>
                  <dt className="text-gray-500 text-xs">Ethnicity</dt>
                  <dd className="font-medium text-gray-900 mt-0.5">{client.ethnicity || "—"}</dd>
                </div>
                <div>
                  <dt className="text-gray-500 text-xs">Gender</dt>
                  <dd className="font-medium text-gray-900 mt-0.5">{client.gender || "—"}</dd>
                </div>
              </dl>
            </div>
              ...
              <p className="text-sm text-gray-700">
                {client.addressLine1 ? (
                  <>
                    ...
                    {(client.postTown || client.city) && <><br />{client.postTown || client.city}</>}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-xs space-y-4 h-fit">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <span className="font-medium text-gray-900">{client.phone || client.telephoneNo || "—"}</span>
              </div>
              <div>
